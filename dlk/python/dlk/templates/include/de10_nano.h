...
struct Csr {
    ...
    static constexpr uint32_t statusRegister = 50;
};
...
Parameters calcParameters(uint32_t inputHeight, uint32_t inputWidth, uint32_t inputChannels, uint32_t inputTileWidth, uint32_t inputTileHeight,
    uint32_t outputChannels, uint32_t kernelHeight, uint32_t kernelWidth, uint32_t inputAddress, uint32_t kernelAddress, uint32_t outputAddress) {
  ...
  constexpr uint32_t avalonDataWidth = 128;
...
void RunTCA(unsigned long input_addr, unsigned long output_addr, const T_UINT k_data_packed[],
  BIN_CONV_OUTPUT th_data[], unsigned in_w, unsigned in_h, unsigned in_c_by_word, unsigned nbits_in_data,
  ...
  k_data_mem.Write(k_data_packed, k_size);
    ...
    auto tileWidth = 32u;
    auto tileHeight = 32u;
    auto p = calcParameters(in_h, in_w, in_c_by_word, tileWidth, tileHeight, out_c, k_h, k_w, input_addr, KERNEL_ADDR, output_addr);
